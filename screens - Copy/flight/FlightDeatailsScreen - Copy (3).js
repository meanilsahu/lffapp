...
const FlightDeatailsScreen = () => {
  ...
  const [minDateAdt, setMinDateAdt] = useState(null);
  const [maxDateAdt, setMaxDateAdt] = useState(new Date());
  
  const setMinMaxDatesAdt = () => {
    const currentDate = new Date();
    const minDOBDate = new Date();
    minDOBDate.setFullYear(currentDate.getFullYear() - 100); 
    const maxDOBDate = new Date();
    maxDOBDate.setFullYear(currentDate.getFullYear() - 18); 

    setMinDateAdt(minDOBDate);
    setMaxDateAdt(maxDOBDate);
  };


  const toggalDatePicker=()=>{
  ...
  const onChange=({type},selectedDate)=>{
    if(type=="set"){
      ...
      if(Platform.OS==="android"){
        toggalDatePicker();
        setDateOfBirth(dateformateDOB(currentDate));
      }  
              ...
              {passengerInfo.map((passenger, i) => (
                <View key={i} style={{marginBottom: 20,flexDirection:'row',flexWrap: 'wrap'}}>
                  ...
                  />                  
                      ...
                      {!showPicker && (  
                      <Pressable onPress={()=>{toggalDatePicker();setMinMaxDatesAdt();}}>
                  <TextInput
                      style={{
                        margin: 10,
                        height: 42,
